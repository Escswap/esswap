...
describe('v1SwapArguments', () => {
  const USDC_WBNB = new MockV1Pair('1000000', new TokenAmount(USDC, '1000000'))
  ...
  it('exact eth to token', () => {
    const trade = Trade.exactIn(new Route([USDC_WBNB], BSC), CurrencyAmount.ether('100'))
  ...
  it('exact token to eth', () => {
    const trade = Trade.exactIn(new Route([USDC_WBNB], USDC, BSC), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('exact token to token', () => {
    const trade = Trade.exactIn(new Route([USDC_WBNB, DAI_WBNB], USDC), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('eth to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WBNB], BSC), new TokenAmount(USDC, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('token to exact eth', () => {
    const trade = Trade.exactOut(new Route([USDC_WBNB], USDC, BSC), CurrencyAmount.ether('100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60
  ...
  it('token to exact token', () => {
    const trade = Trade.exactOut(new Route([USDC_WBNB, DAI_WBNB], USDC), new TokenAmount(DAI, '100'))
    const result = v1SwapArguments(trade, {
      recipient: TEST_RECIPIENT_ADDRESS,
      allowedSlippage: new Percent('1', '100'),
      ttl: 20 * 60

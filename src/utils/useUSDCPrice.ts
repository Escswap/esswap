import { ChainId, Currency, currencyEquals, JSBI, Price, WBNB } from 'sb1swap-sdk'
...
export default function useUSDCPrice(currency?: Currency): Price | undefined {
  ...
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    () => [
      ...
      [chainId ? WBNB[chainId] : undefined, chainId === ChainId.MAINNET ? USDC : undefined]
    ],
  ...
  return useMemo(() => {
    ...
    if (ethPairState === PairState.EXISTS && ethPair && usdcEthPairState === PairState.EXISTS && usdcEthPair) {
      if (usdcEthPair.reserveOf(USDC).greaterThan('0') && ethPair.reserveOf(WBNB[chainId]).greaterThan('0')) {
